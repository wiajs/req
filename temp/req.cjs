// @wiajs/req v1.2.0-alpha.1 Copyright (c) 2022 [object Object] and contributors
'use strict';
...
const URLSearchParams = url$1.URLSearchParams;
...
class HttpAgent extends http.Agent {
  constructor(options) {
    ...
    this.proxy = typeof proxy === 'string' ? new node_url.URL(proxy) : proxy;
    this.proxyRequestOptions = proxyRequestOptions || {};
    this.tunnel = opts.tunnel; // http网址，是否使用隧道模式
  }
  ...
  addRequest(request, options) {
    ...
    const m = this;
    request.getHeaders();
    // console.log('addRequest-0', {request, options, headers});
    ...
    // timeout: options.timeout || 0
    request.getHeaders();
    // console.log('addRequest-1', {request, options, headers});
  ...
  createConnection(options, callback) {
    ...
    if (!m.tunnel) {
      ...
      if (m.proxy.username || m.proxy.password) {
        ...
        opt.headers['proxy-authorization'] = `Basic ${base64}`;
      }
      if (m.proxy.protocol === 'https:') opt.servername = m.proxy.hostname;
      console.log('createConnection', {
        options,
        opt,
        proxy: m.proxy
      ...
      // 连接代理服务器
      const request = (m.proxy.protocol === 'http:' ? http : https).request(opt);
      request.once('connect', (response, socket, head) => {
        console.log('connect', {
          response,
          head
...
/**
 * come form 'https://github.com/follow-redirects/follow-redirects'
...
// const assert = require('assert');
// const debug = require('debug')('req:redirect');
...
function RedirectableRequest(options, responseCallback) {
  ...
  if (responseCallback) {
    m.on('response', responseCallback);
  }

  // React to responses of native requests
  this._onNativeResponse = function (response) {
    m._processResponse(response);
  };

  // Perform the first request
  m._performRequest();
}
RedirectableRequest.prototype = Object.create(stream.Writable.prototype);
...
RedirectableRequest.prototype._processResponse = function (response) {
  ...
  m === null || m === void 0 ? void 0 : (_m$debug = m.debug) === null || _m$debug === void 0 ? void 0 : _m$debug.call(m, 'redirecting to', {
    redirectUrl
  });
  ...
  Object.keys(protocols).forEach(function (scheme) {
    ...
    function request(input, options, callback) {
      ...
      if (isString(input)) {
        var parsed;
        try {
          parsed = urlToOptions(new url$1.URL(input));
        } catch (err) {
        ...
        input = parsed;
      } else if (url$1.URL && input instanceof url$1.URL) {
      ...
      if (m !== null && m !== void 0 && m.debug) {
        m.debug('request', {
          options
        });
        options.debug = m.debug;
      }
...
function dispatchBeforeRedirect(options) {
  var _this$debug;
  this === null || this === void 0 ? void 0 : (_this$debug = this.debug) === null || _this$debug === void 0 ? void 0 : _this$debug.call(this, 'dispatchBeforeRedirect', {
...
function setProxy(options, configProxy, location) {
  ...
  m === null || m === void 0 ? void 0 : (_m$debug = m.debug) === null || _m$debug === void 0 ? void 0 : _m$debug.call(m, 'setProxy', {
    options
  });
  options.beforeRedirects.proxy = redirectOptions => {
    // Configure proxy for redirected request, passing the original config proxy to apply
    // the exact same logic as if the redirected request was performed by axios directly.
    setProxy.call(m, redirectOptions, configProxy, redirectOptions.href);
  };
}
...
function httpAdapter(config) {
  ...
  return new Promise(function dispatchHttpRequest(resolvePromise, rejectPromise) {
    ...
    m === null || m === void 0 ? void 0 : (_m$debug2 = m.debug) === null || _m$debug2 === void 0 ? void 0 : _m$debug2.call(m, 'dispatchHttpRequest', {
      isHttpsRequest,
      config,
      agent: options.agent
    });
    if (config.transport) {
      ...
      transport = isHttpsRequest ? https : http$1;
    } else {
    ...
    // Create the request
    req = transport.request.call(m, options, res => {
      var _m$debug3;
      m === null || m === void 0 ? void 0 : (_m$debug3 = m.debug) === null || _m$debug3 === void 0 ? void 0 : _m$debug3.call(m, 'dispatchHttpRequest', {
        options,
        res
      });
...
function dispatchRequest(config) {
  ...
  return adapter.call(this, config).then(function onAdapterResolution(response) {
    throwIfCancellationRequested(config);
...
class Axios {
  ...
  debug(...arg) {
    if (Axios.debug) console.log(...arg);
  }
...
Axios.debug = process.env.NODE_DEBUG && /\bwiajs.req\b/.test(process.env.NODE_DEBUG);
