...
const log = new Log({env: 'wia.redirect', pre: 'RedirectReq'});
...
export default class RedirectReq extends Duplex {
  ...
  write(data, encoding, callback) {
    ...
    log.debug('write', {data, encoding, callback});

    // Writing is not allowed if end has been called
    if (m._ending) throw new WriteAfterEndError();

    // Validate input and shift parameters if necessary
    if (!utils.isString(data) && !utils.isBuffer(data))
    ...
    if (m._requestBodyLength + data.length <= m._options.maxBodyLength) {
      ...
      m._requestBodyBuffers.push({data, encoding});
  ...
  processResponse(response) {
    ...
    log.debug('processResponse', {statusCode, headers: response.headers});

    if (
      !location ||
      m._options.followRedirects === false ||
      statusCode < 300 ||
      statusCode >= 400
    ) {
      // 非重定向，返回给原始回调处理
    ...
    try {
      ...
      m.emit('error', new RedirectionError({cause}));
      return;
    }
  ...
  pipe(dest, opts) {
    ...
    if (m.response) {
      if (m._destdata) {
        m.emit(
          'error',
          new Error('You cannot pipe after data has been emitted from the response.')
        );
      } else if (m._ended) {
