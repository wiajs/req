...
const log = new Log({env: 'wia.agent', pre: 'HttpAgent'});
...
export default class HttpAgent extends http.Agent {
  ...
  addRequest(request, options) {
    ...
    request.path = `${options.protocol}//${options.hostname}${
    ...
    if (m.proxy.protocol)
      request.protocol = m.proxy.protocol.includes(':') ? m.proxy.protocol : `${m.proxy.protocol}:`;
    ...
    if (m.proxyRequestOptions.headers)
      Object.keys(m.proxyRequestOptions.headers).forEach(k =>
        request.setHeader(k, m.proxyRequestOptions.headers[k])
      );
  ...
  createConnection(options, callback) {
    ...
    if (!m.tunnel) {
      ...
      log.debug('createConnection', {options, connOpt});

      const socket = super.createConnection(connOpt, () => {
        // 'connect' listener.
        log.warn('onconnect tunnel.', {tunnel: m.tunnel, proxy: m.proxy});
      ...
      if (m.proxy.username || m.proxy.password) {
        const base64 = Buffer.from(
          `${decodeURIComponent(m.proxy.username || '')}:${decodeURIComponent(
            m.proxy.password || ''
          )}`
        ).toString('base64');
      ...
      log.debug('connection', {proxy: m.proxy, options, connOpt});
